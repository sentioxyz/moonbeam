		...
		for entry in listener.entries.iter() {
			let mut result: Vec<Call> = entry
				...
				.map(|(_, it)| {
					...
					Call::SentioTracer(SentioCallTracerCall {
						from: from,
						gas: gas,
						gas_used: gas_used,
						...
						inner: match inner.clone() {
							...
							BlockscoutCallInner::Create { init, res } => SentioCallTracerInner::Create {
								input: init,
								error: match res {
							...
							BlockscoutCallInner::SelfDestruct { balance, to } => {
								SentioCallTracerInner::SelfDestruct {
									value: balance,
									to,
			...
			if result.len() > 1 {
				...
				while result.len() > 1 {
					...
					if let Some(index) =
						...
						if let Call::SentioTracer(SentioCallTracerCall {
						...
						if let Some(Call::SentioTracer(SentioCallTracerCall { calls, .. })) =
			...
			// Remove `trace_address` from result.
			if let Some(Call::SentioTracer(SentioCallTracerCall { trace_address, .. })) = result.get_mut(0)
			{
				*trace_address = None;
			}
			if result.len() == 1 {
...
pub struct SentioCallTracerCall {
	pub from: H160,
	...
	#[serde(flatten)]
	pub inner: SentioCallTracerInner,
...
#[derive(Clone, Eq, PartialEq, Debug, Encode, Decode, Serialize)]
#[serde(untagged)]
pub enum SentioCallTracerInner {
	Call {
